...
public class ResetPasswordConfirmAction extends ActionSupport implements SessionAware {
	...
	private List<String> userExistsErrorMessageList = new ArrayList<String>();
	...
	public String execute() {
		...
		boolean userExists = userInfoDAO.isExistsUserInfo(loginId, password);
		if (!(userExists)) {
			userExistsErrorMessageList.add("パスワードが異なります。");
		}
		...
		if (loginIdErrorMessageList.size() == 0 && passwordErrorMessageList.size() == 0
				...
				&& newPasswordIncorrectErrorMessageList.size() == 0 && userExistsErrorMessageList.size() == 0) {
			...
			if (loginIdErrorMessageList.size() == 0 && passwordErrorMessageList.size() == 0) {
				session.put("userExistsErrorMessageList", userExistsErrorMessageList);
			} else {
